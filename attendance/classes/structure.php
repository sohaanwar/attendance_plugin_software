...
//require_once(dirname(__FILE__) . '/student_absence.php');///
...
class mod_attendance_structure {
    ...
    public function take_from_form_data($formdata) {
        ...
        $sesslog = array();
        $absence=array();/////////sooha
        $sts=array_keys( (array)$this->get_statuses());
        $sql="SELECT c.id
    
                       
                        from mdl_attendance_sessions s
                            
                        INNER JOIN mdl_attendance a
                            on s.attendanceid = a.id and s.id=:sessid
                        INNER JOIN mdl_course c
                            on a.course= c.id 
                         ";
         $courseid=$DB->get_record_sql($sql,array('sessid'=>$this->pageparams->sessionid));//////soha
        ...
        foreach ($formdata as $key => $value) {
            ...
            if (substr($key, 0, 7) == 'remarks') {
                ...
                if($sesslog[$sid]->statusid==$sts[3])////soooha
                {
                    $absence[$sid]=new stdClass();

                    $absence[$sid]->student=$sid;
                     $absence[$sid]->course=$courseid->id; 
                    $absence[$sid]->tobenotified=0;
                   $absence[$sid]->numberofabsences=1;
                }
            }
        }
                ...
                if (array_key_exists($log->studentid, $dbsesslog)) {
                    ...
                    $DB->insert_record('attendance_log', $log);
                     ...
                     foreach ($absence as $a) {
                       ...
                       $DB->insert_record('number_of_student_absences', $a, false);
